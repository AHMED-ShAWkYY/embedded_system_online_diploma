...
typedef struct
{
	EXTI_GPIO_Mapping_T EXTI_Pin; 		  //specifies the EXTI GPIO_Mapping
							     		 // this parameter must be set based on @ref EXTI define
	uint8_t 			trigger_case;   //specifies the EXTI trigger case
							      	   //this parameter must be set based on @ref EXTI trigger_case
	uint8_t 			IRQ_EN		; //enable or disable IRQ enable the mask and enable the NVIC interrupt
									 //this parameter must be set based on @ref EXTI IRQ define
	void (*P_IRQ_Callback)(void);   //define which function that will be called when the interrupt happens

}EXTI_Pin_Config_t;


/*********************************/
/***   MACROS CPNFIGURATION   ***/
/********************************/
//EXTI
#define EXTI0 	0
#define EXTI1 	1
